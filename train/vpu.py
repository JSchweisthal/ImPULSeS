"""
source: https://github.com/HC-Feynman/vpu
"""
...
def run_vpu(config, loaders, NetworkPhi):
    ...
    for epoch in range(config.epochs):
        ...
        val_var, test_acc, test_auc, test_f1 = evaluate(model_phi, x_loader, test_loader, val_p_loader, val_x_loader, epoch,
        ...
        if is_val_var_lowest:
            ...
            test_f1_of_best_val = test_f1
            epoch_of_best_val = epoch
        save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model_phi.state_dict(),
            'optimizer': opt_phi.state_dict(),
        }, is_val_var_lowest, is_test_acc_highest, config=config)

    # inform users model in which epoch is finally picked
    print('Early stopping at {:}th epoch, test AUC : {:.4f}, test acc: {:.4f}, test F1: {:.4f}'.format(epoch_of_best_val,
...
def evaluate(model_phi, x_loader, test_loader, val_p_loader, val_x_loader, epoch, phi_loss, var_loss, reg_loss):
    """
    ...
    print('Train Epoch: {}\t phi_loss: {:.4f}   var_loss: {:.4f}   reg_loss: {:.4f}   Test accuracy: {:.4f}   Val var loss: {:.4f}   Test f1: {:.4f}' \
          .format(epoch, phi_loss, var_loss, reg_loss, test_acc, val_var, test_f1))
    return val_var, test_acc, test_auc, test_f1
